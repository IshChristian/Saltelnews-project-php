...
$sql=mysqli_query($con, "SELECT * FROM (SELECT * FROM message WHERE message.receiver='$sname' AND message.view='0' ORDER BY timestamp DESC LIMIT 10) AS
message LEFT JOIN member ON member.m_id=message.sid GROUP BY message.sender ORDER BY message.timestamp DESC;");
$viewed=mysqli_query($con, "SELECT * FROM (SELECT * FROM message WHERE message.receiver='$sname' AND message.view='1'  ORDER BY timestamp DESC LIMIT 10)
AS message LEFT JOIN member ON member.m_id=message.sid GROUP BY message.sender ORDER BY timestamp DESC;");
// if(mysqli_num_rows($sql)>0){
while($row2=mysqli_fetch_array($sql)){
$user=$row2['rid'];
?>
<a href="message.php?uid=<?php echo $row2['sid'] ?>&sid=<?php echo $sendername ?>&mid=<?php echo $row2['id'] ?>" class="text-white text-decoration-none">
<div class="friend-drawer d-flex friend-drawer--onhover">
<div class="dp-con">
<img class="profile-image rounded-circle" style="max-width: 50px; max-height: 50px;" src="msg_assets/<?php echo $row2['image'] ?>" alt="">
<?php
...
<div class="text ml-3">
<h6><?php echo $row2['sender'] ?></h6>
<?php
if($row2['view'] == "0"){
?>
<p class="ellipsis"><?php echo $row2['message'] ?></p>
<?php
}else{
?>
<p class="text-muted ellipsis"><?php echo $row2['message'] ?></p>
<?php
}
?>
<p class="text-muted"><?php echo $row2['timestamp'] ?></p>
</div>
<!-- <div class="time" style="float: right">
<span class="time text-muted small"><?php echo $row2['timestamp'] ?></span> 
</div> -->
</div>
</a>
<hr>  
...
$user=$row['rid'];
?>
<a href="message.php?uid=<?php echo $row['m_id'] ?>&sid=<?php echo $sendername ?>&mid=<?php echo $row['id'] ?>" class="text-white text-decoration-none">
<div class="friend-drawer d-flex friend-drawer--onhover">
<div class="dp-con">
...
<div class="text ml-3">
<h6><?php echo $row['sender'] ?></h6>
<?php
if($row['view'] == "0"){
?>
<p class="ellipsis"><?php echo $row['message'] ?></p>
<?php
}else{
?>
