...
<body>
    ...
    <?php
    $sql=mysqli_query($con, "SELECT * FROM add_news ORDER BY date DESC LIMIT 1;");
    while($rows=mysqli_fetch_array($sql)){
        $Nid=$rows['n_id'];
        ?>
    <!-- ##### Catagory Featured Area Start ##### -->
    <div class="catagory-featured-post bg-overlay clearfix" style="background-image: url(admin/img/<?php echo $rows['image'] ?>)">
                    ...
                    <div class="intro-news-tab">
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            ...
                            while($Nrows=mysqli_fetch_array($sql)){
                                $Cname=$Nrows['name'];
                                ?>
                                <div class="tab-pane fade show active" id="nav-<?php echo $Nrows['id'] ?>" role="tabpanel" aria-labelledby="nav<?php echo $Nrows['id'] ?>">
                                    
                            
                                <div class="row">
                                <?php
                                $sql=mysqli_query($con, "SELECT * FROM add_news ORDER BY n_id DESC LIMIT 7;");
                                while($rows=mysqli_fetch_array($sql)){
                                    $Nid=$rows['n_id'];
                                    ?>
                                    <!-- Single News Area -->
